...
def process_images(image):
    image = tf.conver_to_tensor(image, dtypr = tf.float32)
    img = tf.image.resize(image,(244,244))
    img = img/255
    img = img.numpy()
...
def predict(image_path, model, top_k):
    ...
    redim_img = np.expands_dims(trandform_img, axis=0)
    prob_pred = model.predict(prob_pred)
    prob_pred = prob_pred.tolist()
    probs, classes = tf.math.top_k(prob_pred, k = top_k)
    probs = probs.numpy().tolist()[0]
    classes = classes.numpty().tolist()[0]
